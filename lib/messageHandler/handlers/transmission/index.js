...
const TransmissionHandler = (_ref) => {
  let {
    realm
  } = _ref;
  ...
  const handle = (client, message) => {
    ...
    if (destinationClient) {
      ...
      // messages.
      const ignoredTypes = [_enums.MessageType.LEAVE, _enums.MessageType.EXPIRE];

      if (!ignoredTypes.includes(type) && dstId) {
        realm.addMessageToQueue(dstId, message);
      } else if (type === _enums.MessageType.LEAVE && !dstId) {
